...
class VikrentcarViewStandbyorder extends JView {
	function display($tpl = null) {
		...
		$saldo= vikrentcar::getSaldoUser($order[0]['ujid']);
		...
		$q= "INSERT INTO `#__vikrentcar_transaccion` (`fecha`, `id_factura`) VALUES('".$mysqlDateTime."','".$order[0]['id_factura']."')";
		$dbo->setQuery($q);
		$dbo->Query($q);
		...
		//vikrentcar 1.5
		if($order[0]['hourly'] == 1) {
			$q = "SELECT * FROM `#__vikrentcar_dispcosthours_master` WHERE `id`='" . $order[0]['idtar'] . "';";
		}else {
			$q = "SELECT * FROM `#__vikrentcar_dispcost_master` WHERE `id`='" . $order[0]['idtar'] . "';";
		}
		//
		$dbo->setQuery($q);
		$dbo->Query($q);
		...
		$this->assignRef('format', $id_transaccion);
		$this->assignRef('saldo', $saldo);
