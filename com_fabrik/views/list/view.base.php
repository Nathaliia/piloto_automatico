...
class FabrikViewListBase extends JView
{
	...
	public function display($tpl = null)
	{
		...
		if (!$model->canView())
		{
			...
			//$url = JRoute::_('index.php?option=com_users&view=login&Itemid=696&return=' . base64_encode('index.php?option=com_fabrik&view=list&listid=35&Itemid=697&lang=es'));
			//$app->redirect($url, $message);
			//
			$user = JFactory::getUser();
			if($user->id=='' || $user->id==NULL){

				$Itemid	 = JRequest :: getString('Itemid', '', 'request');
			    $app->redirect(JURI::root().'index.php?option=com_users&view=login&Itemid=341&return=' . base64_encode(JURI::root().'index.php?option=com_fabrik&view=list&listid='.$item->id.'&Itemid='.$Itemid), JText::_("COM_FABRIK_MESSAGE_RESTRICACCESS"));
			}else{
